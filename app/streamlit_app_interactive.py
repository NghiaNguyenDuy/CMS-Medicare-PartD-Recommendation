...
def calculate_distance_penalty(miles, penalty_per_mile=50.0):
    """
    Calculate distance penalty in dollars.
    Matches training data: $50 per mile.
    
    Args:
        miles (float): Distance in miles
        penalty_per_mile (float): Penalty rate ($/mile)
    
    Returns:
        float: Distance penalty in dollars
    """
    return miles * penalty_per_mile
...
def get_state_county_codes(state_name, county_name):
    ...
    county_code = str(county_result['county_code'].iloc[0])
...
def rank_plans(plans_df, beneficiary_profile, model_data):
    """
    ...
    for _, plan in plans_df.iterrows():
        # Estimate OOP cost (simplified - in production, use actual drug list)
        estimated_oop = beneficiary_profile['total_rx_cost_est'] * 0.3  # 30% patient responsibility
        ...
        features = {
            ...
            'distance_miles': plan.get('nearest_preferred_miles', plan.get('distance_miles', 10.0)),
    ...
    plans_df = plans_df.sort_values('score', ascending=False).reset_index(drop=True)
...
def main():
    ...
    distance_penalty_rate = st.sidebar.number_input(
        "Distance penalty ($/mile)",
        min_value=10.0,
        max_value=100.0,
        value=50.0,
        step=10.0,
        help="How much you value being closer (higher = prefer local plans)",
        disabled=not include_nearby
    )
    ...
    if get_recommendations:
        ...
        beneficiary_profile = {
            ...
            'zip_code': zip_code_input if zip_code_input else None,
            'num_drugs': num_drugs,
        ...
        with st.spinner("🔍 Searching for plans..."):
            if include_nearby:
                plans_df, search_info = get_plans_with_fallback(
                    ...
                    zip_code_input if zip_code_input and len(zip_code_input) == 5 else user_default_zip,
        ...
        with st.spinner("Ranking plans using ML model..."):
            ranked_plans = rank_plans(plans_df, beneficiary_profile, model_data)
        ...
        st.markdown("---")
        st.subheader("🏆 Top 5 Recommended Plans")
        ...
        for idx, (_, plan) in enumerate(top_5.iterrows(), 1):
            with st.container():
                ...
                with col3:
                    st.markdown(f'<p class="cost-highlight">${plan["total_annual_cost"]:,.2f}</p>', unsafe_allow_html=True)
                    st.caption("Total Annual Cost")
                ...
                col_c.metric("ML Ranking Score", f"{plan['score']:.2f}")
                ...
                if not plan.get('is_local', True):
                    ...
                    # Distance penalty
                    distance_penalty = calculate_distance_penalty(distance, distance_penalty_rate)
                    st.markdown(f"**🚗 Distance Penalty:** ${distance_penalty:,.0f}/year at ${distance_penalty_rate:.0f}/mile")
                else:
                ...
                col_f1.metric("Generic %", f"{plan['formulary_generic_pct']:.1f}%")
                col_f2.metric("PA Rate", f"{plan['formulary_pa_rate']:.1f}%")
                col_f3.metric("Restrictiveness", f"{plan['formulary_restrictiveness']}/100")
                
                # Warnings
                warnings = []
                if is_insulin_user and plan['formulary_pa_rate'] > 50:
                ...
                if plan['formulary_restrictiveness'] > 50:
        ...
        fig.update_layout(
            barmode='stack',
            title='Premium vs. Drug Costs Breakdown',
        ...
        report_df = top_5[[
            ...
            'estimated_annual_oop', 'total_annual_cost', 'score',
